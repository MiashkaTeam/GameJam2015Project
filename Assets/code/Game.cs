...
public class Game : MonoBehaviour {
	...
	private void renderPathRowFor(int x) {
		...
		for (int y = 0; y < data.Length; y++) {
			...
			data[y].transform.position = new Vector3((this.players[0].round - 1) * this.map.roundWidth + x * Map.PlatformSize.x, y * 3.0f, 0);
		}
	...
	void Start () {
		...
		this.players[0].TileChanged += OnTileChanged;
		this.renderPathRowFor(this.players[0].mapPosX);
		this.renderPathRowFor(this.players[0].mapPosX + 1);
	...
	void OnTileChanged (Player player)
	{
	...
	void OnGUI() {
		...
		GUI.Label (new Rect (170.1f, 10.8f, 90.15f, h), "<size=14>Distance</size>");
		GUI.Label (new Rect (230.1f, 10.8f, 90.15f, h), "<size=14>Tile</size>");
		GUI.Label (new Rect (260.1f, 10.8f, 90.15f, h), "<size=14>Round</size>");
		...
		for (int i = 0; i < this.players.Count; i++) {
			...
			GUI.Label (new Rect (230.1f, 10.8f + h * n, 90.15f, h), "<size=12>"+this.players[i].mapPosX.ToString()+"</size>");
			GUI.Label (new Rect (260.1f, 10.8f + h * n, 90.15f, h), "<size=12>"+this.players[i].round.ToString()+"</size>");
		}
	}
